    ...
    public async create(req: Request, res: Response): Promise<Response> {
        ...
        try {
            ...
            if(user && products) {  
                ...
                //TODO: USAR UNA CONSTANTE PARA EL ARRAY DEL SELECT
                ...
                if (selectedProducts.length !== productsId.length) {
                    ...
                    const productsNotIncluded: number[] = productsId
                        .filter(productId => !selectedProducts.some(product => product.id === productId));
                    return res.status(400).json({ 
                        message: `ids[ ${productsNotIncluded} ] no corresponden a productos existentes` });
                }
                ...
                selectedProducts.forEach(selectedProduct => {
                    ...
                    if (matchingProduct) {
                        if (matchingProduct.quantity > selectedProduct.quantity) {
                            return res.status(400).json({ 
                                message: `No hay suficientes existencias para el producto con ID ${selectedProduct.id}` });
                        }
                    }
                });
